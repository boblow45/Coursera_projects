class FastCollinearPoints(object):
    def __init__(self, points):
        ...
        for element in range(len(points)):
            ...
            self.points[element].append(array)
            # points is an array of arrays. The array inside the array is made up of the following way
            # [x-point, y-point, slope with respect to point of interest]
    ...
    def FastCollinearPoints(self):
        ...
        for j in range(self.N):
            ...
            for element in range(j+1, self.N):
                slope = (self.points[element][1] - self.points[j][1])/(self.points[element][0] - self.points[j][0])
                self.points[element][2] = slope
